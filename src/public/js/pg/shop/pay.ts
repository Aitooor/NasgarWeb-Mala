const recaptcha = document.querySelector("div#recaptcha") as HTMLDivElement;
let captcha;
...
let onResize = (isMobile: boolean) => {};

const total_cash = document.querySelector(".total-cash") as HTMLDivElement;
const url_href = new URL(location.href);

if (!url_href.searchParams.get("total")) window.open("/shop/cart", "_self");
...
function onLoadCaptcha() {
  ...
  const c_0 = recaptcha.querySelector("div:first-child") as HTMLDivElement;
  const c_1 = c_0.querySelector("iframe:first-of-type") as HTMLIFrameElement;
...
globalThis.formValidator = (e) => {
  const nick = form.querySelector('input[name="nick"]') as HTMLInputElement;
  const payment = form.querySelector('input[name="payment"]:checked') as HTMLInputElement;
  ...
  if (nick.value.length <= 0) {
    ...
    const inp = form.querySelector('input[name="shop_cart"]') as HTMLInputElement;

    inp.value = localStorage["shop_cart"] + ";" + localStorage["shop_cupon"];
